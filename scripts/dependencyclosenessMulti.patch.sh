...
for i in {0..9}; do cat $pre/pkgcmtslist.$target.filter2.$i | perl c2b2upstreams.perl dplyrVSHmisc/regexPkgs.all 16 /data/basemaps/c2bFullF >$pre/pcbPkg.$target.$i    2>$pre/pcbPkg.$target.$i.err & done
wait
