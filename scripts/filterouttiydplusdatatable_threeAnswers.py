...
#coll2 = db['Posts_title_body_answer_0' + 'tidy_readr_tibble']
#coll3 = db['Posts_title_body_answer_5' + 'tidy_readr_tibble']
#coll4 = db['Posts_title_body_answer_20' + 'tidy_readr_tibble']
coll2 = db['Posts_title_body_answer_0' + targets[0]]
coll3 = db['Posts_title_body_answer_5' + targets[0]]
coll4 = db['Posts_title_body_answer_20' + targets[0]]
...
for prj in prj2timestamp:
    if len(excludes) != 0:
        results = coll.find({field1: {'$regex': target, '$not': re.compile(
            excludes[0]), '$options': 'i'}, 'PostTypeId': "2"}, {'_id': 0}, no_cursor_timeout=True)
    else:
        results = coll.find(
            {field1: {'$regex': target, '$options': 'i'}, 'PostTypeId': "2"}, {'_id': 0}, no_cursor_timeout=True)
#    print(len(list(results)))
    ...
    if len(excludes) != 0:
        ...
        results = coll.find(
            {field2: {'$regex': target, '$options': 'i'}, 'PostTypeId': "2"}, {'_id': 0}, no_cursor_timeout=True)
#    print(len(list(results)))
    for i in results:
        #print("at least have one in results")
        if i['Id'] not in Id_list:
